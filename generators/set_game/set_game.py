...
class SetGame:
    def __init__(self, verbose=0):
        ...
        im = mpimg.imread(os.path.join(_dirname, "all-cards.png"))
    ...
    class State:
        ...
        def advance(self, env, action=0):
            ...
            if action == 0:  # measure attribute
                self.partial_attributes[self.attribute_ind] = self.focus_attributes[
                    self.attribute_ind
                ]
    ...
    def set_rewards(self, rewards, verbose=0):
        ...
        if verbose:
            print(
                "Rewards: set=%d, notset=%d, swap=%d, default=%d"
                % (
                    self.set_reward,
                    self.notset_reward,
                    self.swap_reward,
    ...
    def init_state(self, num_cards=12, verbose=0, shuffle=True):
        ...
        while True:
            ...
            if (
                len(set(self.triples[set_index]) & set([card_coord[j] for j in inds]))
                == 0
            ):
                break
    ...
    def show_state(self):
        ...
        self.show_cards(
            self.state.dealt_cards, 3, int(np.ceil(self.state.num_cards / 3)), labels
        )
    ...
    def image_of_card(self, row, col):
        topleft = np.array(
            [
                self.leftmargin + row * (self.height + self.hspace),
    ...
    def attributes_of_card(self, row, col):
        return (
            self.number[row % 3],
            self.color[int(col / 3)],
            self.pattern[col % 3],
            self.shape[int(row / 3)],
        )
    ...
    def triple_is_set(self, triple):
        ...
        BC = self.tabulate_features_for_pair(triple[1], triple[2])
        is_set = (
            self.attribute_is_good(AB[0], AC[0], BC[0])
            and self.attribute_is_good(AB[1], AC[1], BC[1])
            and self.attribute_is_good(AB[2], AC[2], BC[2])
            and self.attribute_is_good(AB[3], AC[3], BC[3])
        )
        return is_set
    ...
    def generate_data(self):
        ...
        print(
            "Probability of SET! (in %d samples): %f (1/79=%f)"
            % (n, sum(y) / len(y), 1 / 79)
        )
        return X, y, triples
